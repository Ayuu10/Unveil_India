...
def create():
    ...
    conn = sqlite3.connect("static/GST Travels.db")  # Connect to the SQLite database
    ...
    # Create find table (associative table between city and tag)
    cur.execute("CREATE TABLE IF NOT EXISTS find( cid char(3) REFERENCES city(cid), tid char(3) REFERENCES tag(tid));")
    # Create destination table with various details
    cur.execute("""CREATE TABLE IF NOT EXISTS destination(
                cid char(3) REFERENCES city(cid), 
                Budget int,
                SightSeeing varchar(200),
                byroad varchar(200),
                byrail varchar(200),
                byair varchar(200),
                Hotels5star varchar(200),
                Hotels3star varchar(200),
                Hotels1star varchar(200),
                Restaurents5star varchar(200),
                Restaurents3star varchar(200),
                Restaurents1star varchar(200),
                NearbySights varchar(200)
                );""")
    conn.commit()  # Commit the changes
    conn.close()  # Close the database connection

def insert_preset():
    """Insert preset data into the database tables."""
    conn = sqlite3.connect("static/GST Travels.db")  # Connect to the SQLite database
    cur = conn.cursor()  # Create a cursor object to execute SQL commands
    # Insert data into the tag table
    cur.execute("""INSERT INTO tag VALUES
                ('T01',"Beach Holidays"),('T02',"Adventure Tour"),('T03',"Cultural Experience"),
                ('T04',"Wildlife Safaries"),('T05',"Mountain Expeditions"),('T06',"Urban Exploration"),
                ('T07',"Wellness Retreats"),('T08',"Historical Journeys"),('T09',"Eco Tourism"),
                ('T10',"Road Trips"),('T11',"Pilgrimage");""")
    conn.commit()
    # Insert data into the city table
    cur.execute("""INSERT INTO city VALUES
                ('C01',"Goa"),('C02',"Kovalam (Kerala)"),('C03',"Andaman and Nicobar Islands"),
                ('C04',"Manali (Himachal Pradesh)"),('C05',"Rishikesh (Uttarakhand)"),('C06',"Leh (Ladakh)"),
                ('C07',"Jaipur (Rajasthan)"),('C08',"Varanasi (Uttar Pradesh)"),('C09',"Kolkata (West Bengal)"),
                ('C10',"Ranthambore (Rajasthan)"),('C11',"Jim Corbett (Uttarakhand)"),('C12',"Kanha (Madhya Pradesh)"),
                ('C13',"Darjeeling (West Bengal)"),('C14',"Mumbai (Maharashtra)"),('C15',"Delhi"),('C16',"Kerala"),
                ('C17',"Tirupati (Andhra Pradesh)"),('C18',"Agra (Uttar Pradesh)"),('C19',"Munnar (Kerala)"),
                ('C20',"Coorg (Karnataka)"),('C21',"Bangalore (Karnataka)"),('C22',"Haridwar (Uttarakhand)");""")
    conn.commit()
    # Insert data into the find table (associative table)
    cur.execute("""INSERT INTO find VALUES
                ('C01',"T01"),('C02',"T01"),('C03',"T01"),('C04',"T02"),('C05',"T02"),('C06',"T02"),
                ('C07',"T03"),('C08',"T03"),('C09',"T03"),('C10',"T04"),('C11',"T04"),('C12',"T04"),
                ('C06',"T05"),('C04',"T05"),('C13',"T05"),('C14',"T06"),('C15',"T06"),('C09',"T06"),
                ('C05',"T07"),('C01',"T07"),('C16',"T07"),('C07',"T08"),('C08',"T08"),('C18',"T08"),
                ('C19',"T09"),('C10',"T09"),('C20',"T09"),('C06',"T10"),('C21',"T10"),('C07',"T10"),
                ('C08',"T11"),('C22',"T11"),('C17',"T11");""")
    conn.commit()
...
def gettags():
    ...
    conn = sqlite3.connect("static/GST Travels.db")  # Connect to the SQLite database
    cur = conn.cursor()  # Create a cursor object to execute SQL commands
    cur.execute("select tag from tag")  # Execute SQL command to fetch all tags
    value = []
    lis = cur.fetchall()  # Fetch all results
    val = len(lis)
    for _ in range(val):
        value.append(lis[_][0])  # Append each tag to the value list
    conn.close()  # Close the database connection
    return tuple(value)  # Return the tags as a tuple

def getcitys(tag):
    """Retrieve cities associated with a given tag."""
    conn = sqlite3.connect("static/GST Travels.db")  # Connect to the SQLite database
    cur = conn.cursor()  # Create a cursor object to execute SQL commands
    ...
    val = len(lis)
    for _ in range(val):
        value.append(lis[_][0])  # Append each city to the value list
    conn.close()  # Close the database connection
    return tuple(value)  # Return the cities as a tuple

def prestart():
    """Create tables and insert preset data."""
    create()  # Create the tables
    insert_preset()  # Insert the preset data

def get_destination_info(city_name):
    """Retrieve destination information for a given city."""
    conn = sqlite3.connect("static/GST Travels.db")  # Connect to the SQLite database
    cursor = conn.cursor()  # Create a cursor object to execute SQL commands
    cursor.execute(f"SELECT cid FROM city WHERE city ='{city_name}'")  # Get city ID from the city name
    city_id = cursor.fetchone()  # Fetch the city ID
    cursor.execute(f"SELECT * FROM DESTINATION WHERE cid='{city_id[0]}'")  # Get destination info for the city ID
    data = cursor.fetchone()  # Fetch the destination info
    city = re.sub(r'\(.*?\)', '', city_name).strip()  # Remove parentheses from city name
    info = {
        'How to Reach': {
            'By Road': data[3],
            'By Air': data[5],
    ...
    conn.close()  # Close the database connection
    return info  # Return the destination info

# Check if the database and tables exist, if not, create and insert preset data
try:
...
# Main function to print all cities
